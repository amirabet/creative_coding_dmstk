...
```
sketches/06/
├── sketch-06.js          ← The single source file for the entire sketch
...
│   ├── sketch-06.html    ← Default build output
│   ├── orion.html
...
npm start             # alias for: npx canvas-sketch sketch-06.js --open
```

The dev server opens the sketch in a browser with live reload. **Do NOT use `node sketch-06.js`** — it is a browser script bundled by Browserify.
...
```bash
npm run build          # → docs/sketch-06.html  (default, no CONFIG override)
...
## 5. Architecture of `sketch-06.js`
